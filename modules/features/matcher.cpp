...
using namespace DensePoints::Features;

void Matcher::GenerateSeeds()
{
  // Detect keypoints
  DetectKeypoints();
...
void Matcher::DirectEpipolarMatching()
{
  matches_.resize(pairs_list_.size());
#pragma omp parallel for
  for (size_t match_index = 0; match_index < pairs_list_.size(); ++match_index) {
...
void Matcher::GetAllMatches(KeypointImagePair &keypoint_index,
...
void Matcher::TriangulateMatches()
{
  for (size_t view_index = 0; view_index < views_->size(); ++view_index) {
